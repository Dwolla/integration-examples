...
const CreateCustomersPage: NextPage = () => {
    ...
    /**
     * Calls our Next API to create an MX User Record.
     */
    async function createMxCustomer(): Promise<string | undefined> {
        const response = await fetch("/api/mx/users", {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
            },
        ...
        if (!response.ok) return undefined;
        return ((await response.json()) as MXUsersAPIResponse).user?.guid ?? undefined;
    }

    /**
     * Creates a Dwolla and MX Customer/User Record when the form submits.
     */
    async function handleFormSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        const mxUserGuid = await createMxCustomer();

        if (!mxUserGuid) {
            return updateNetworkAlert({
                alert: { severity: "error", message: "No User ID returned from createMxUser()" },
                networkState: NetworkState.NOT_LOADING
            });
        }
        ...
        await router.push({
            pathname: "/connect-mx",
            query: { dwollaCustomerId, mxUserGuid }
        });
    }
